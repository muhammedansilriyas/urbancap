    ...
    <div className="min-h-screen bg-black py-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {wishlist.map(item => (
            <div key={item.id} className="bg-gray-900 rounded-lg shadow-md overflow-hidden border border-gray-800">
              <Link to={`/product/${item.id}`}>
                <img
                  src={item.thumbnail || item.images?.[0] || '/placeholder-game.jpg'}
                  alt={item.name}
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-gray-400 mb-2">{item.brand || 'No Brand'}</p>
                ...
                <div className="flex items-center mb-3">
                  ...
                  <span className="ml-2 text-sm text-gray-300">{item.rating || 0}</span>
                </div>

                <div className="flex items-center justify-between mb-3">
                  <span className="text-2xl font-bold text-white">₹{item.price || 0}</span>
                  <span className={`text-sm ${(item.stock || 0) > 0 ? 'text-green-500' : 'text-red-500'}`}>
                ...
                <div className="flex space-x-2">
                  ...
                  >
                    ...
                    {(item.stock || 0) > 0 ? 'Move to Cart' : 'Out of Stock'}
                  </button>
                </div>
              </div>
            </div>
